...
function calculate_resources(surface, pos_arr)
    ...
    for proto, _ in pairs((surface.map_gen_settings.autoplace_settings.entity or { settings = {} }).settings or {}) do
        if (prototypes.entity[proto] or {}).type == "resource" then
            ...
            if prototypes.named_noise_expression[proto .. "-probability"] then
    ...
    for i, pos in ipairs(pos_arr) do
        ...
        if not stored_results[chunk_id] then
            stored_results[chunk_id] = surface.calculate_tile_properties(properties,
                get_chunk_positions(pos))
        end
        for _, proto in ipairs(resources) do
            if (stored_results[chunk_id]["entity:" .. proto .. ":richness"] or { [pos_i] = 0 })[pos_i] * stored_results[chunk_id]["subsurface_richness_multiplier"][pos_i] > 0
                ...
                if not result[i] then
                    result[i] = { proto, math.ceil(stored_results[chunk_id]
                        ["entity:" .. proto .. ":richness"][pos_i] *
                        stored_results[chunk_id]["subsurface_richness_multiplier"][pos_i]) }
                end
            end
        end
    end
...
function manipulate_resource_entities(surface, area)
    ...
    for chunk in area and dummy_iter() or surface.get_chunks() do
        ...
        local calc_result = surface.calculate_tile_properties(
            { "subsurface_richness_multiplier", "surface_stone_probability_multiplier" },
            get_chunk_positions(chunk.area.left_top))
        for pos_i, e in pairs(entities_in_area) do
            local amount = e.amount
            amount = math.ceil(amount *
                (e.name == "stone" and calc_result.surface_stone_probability_multiplier[pos_i] or calc_result.subsurface_richness_multiplier[pos_i]))
            if amount > 0 then
                e.amount = amount
            else
                e.destroy()
    ...
    for name, _ in pairs((from_surface.map_gen_settings.autoplace_settings.entity or { settings = {} }).settings) do
        if (prototypes.entity[name] or {}).type == "resource" then
            ...
            if from_surface.map_gen_settings.property_expression_names["entity:" .. name .. ":richness"] then
                mgs.property_expression_names["entity:" .. name .. ":richness"] = from_surface.map_gen_settings
                    .property_expression_names["entity:" .. name .. ":richness"]
            end
            if from_surface.map_gen_settings.property_expression_names["entity:" .. name .. ":probability"] then
                mgs.property_expression_names["entity:" .. name .. ":probability"] = from_surface.map_gen_settings
                    .property_expression_names["entity:" .. name .. ":probability"]
            end
        end
    end
    if mgs.autoplace_settings and mgs.autoplace_settings.entity and mgs.autoplace_settings.entity.settings and mgs.autoplace_settings.entity.settings["stone"] then
